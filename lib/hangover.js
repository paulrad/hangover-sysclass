...
;(function  HangOver(root, undefined) {
    ...
    // see http://phpjs.org/functions/dirname/
    $$source = $$parent.filename.replace(/\\/g, '/').replace(/\/[^\/]*\/?$/, '');

    var $$paths = {
        configuration: $$path.join($$source, 'api/configuration/'),
        controllers: $$path.join($$source, 'api/controllers/'),
        helpers: $$path.join($$source, 'api/helpers/'),
        models: $$path.join($$source, 'api/models/'),
        logs: $$path.join($$source, 'logs/'),
        routes: $$path.join($$source, 'api/routes/'),
        tasks: $$path.join($$source, 'api/tasks/'),
        views: $$path.join($$source, 'api/views/')
    };
    ...
    root.Ho = Ho = module.exports = new (function() {
         ...
         * @invoked by `run`
         */
        var $install = function(path, callbacks) {
        ...
        return {
            ...
            config: function(attribute) {
                if (! $$modules['$$configuration']) {
                    Ho.require($file('configuration', 'config.js'), '$$configuration', function(e) {
                        Ho.err(["Unable to find the configuration file.", "Please, add a config.js file in /api/configuration/ directory."]).throw(e);
                    });
            ...
            out: function(message) {
              var c = new Date(),
                i = 0,
                buff = (c.getHours() + ':' + c.getMinutes() + ':' + c.getSeconds() + '.' + c.getMilliseconds()).underline + ' ';
